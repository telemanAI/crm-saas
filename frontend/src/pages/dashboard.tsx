...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    // Solo SUPER_ADMIN può accedere alle route /admin/*
  ...
  const loadDashboardData = async () => {
    ...
    try {
      // Path corretti: senza /api/ iniziale (baseURL già include /api/v1)
  ...
  const openConfigModal = async (tenant: any) => {
    ...
    try {
      // ✅ CRITICAL FIX: Path corretto /admin/tenants/.../config (non /tenants/...)
      const response = await api.get(`/admin/tenants/${tenant.id}/config`);
  ...
  const saveConfig = async () => {
    ...
    try {
      // ✅ CRITICAL FIX: Path corretto /admin/tenants/.../config
      await api.put(`/admin/tenants/${configModal.tenant.id}/config`, tenantConfig);
                    ...
                    <tr key={tenant.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex gap-2 flex-wrap">
                          {/* Pulsanti diretti Import/Export */}
                          <button 
