...
function RoleCard({ icon, title, subtitle, onClick, testid }: any) {
  return (
    <button
      data-testid={testid}
      onClick={onClick}
...
function Field(props: { icon: React.ReactNode; type?: string; placeholder: string; value: string; onChange: (v: string) => void; required?: boolean; trailing?: React.ReactNode; testid?: string }) {
  return (
    <div className=\"relative\">
      <div className=\"absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500\">{props.icon}</div>
      <input
      ...
      />
      {props.trailing && <div className=\"absolute right-3.5 top-1/2 -translate-y-1/2\">{props.trailing}</div>}
    </div>
  );
}
