...
export default function MobilePracticesList() {
  ...
  // Filtra SOLO pratiche MOBILE
  const mobilePractices = practices.filter((p) => p.category === 'MOBILE');

  // Applica filtri di ricerca e stato
  const filtered = mobilePractices.filter((p) => {
    const matchesSearch =
      !search ||
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-white">Pratiche Rete Mobile</h1>
              <p className="text-slate-400 text-sm mt-1">
                {mobilePractices.length} pratiche in totale
              </p>
            </div>
          </div>
          <Link href="/operator/practices/mobile/new">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="flex items-center gap-2 px-5 py-3 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl font-medium transition-colors shadow-lg shadow-cyan-600/20"
            >
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-700 rounded-xl pl-10 pr-4 py-3 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-cyan-500"
            />
          </div>
          <div className="relative">
            ...
            <select
              ...
              className="bg-slate-900 border border-slate-700 rounded-xl pl-10 pr-8 py-3 text-slate-200 focus:outline-none focus:border-cyan-500 appearance-none cursor-pointer"
            >
        ...
        {loading ? (
          ...
          >
            <DeviceMobile className="w-12 h-12 text-slate-600 mx-auto mb-4" />
            <p className="text-slate-400 text-lg">Nessuna pratica mobile trovata</p>
          ...
          <div className="space-y-3">
            {filtered.map((practice, idx) => (
              ...
              >
                <Link href={`/operator/practices/mobile/${practice.id}`}>
                  <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-5 hover:border-cyan-500/50 hover:bg-slate-800/80 transition-all cursor-pointer group">
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-4 text-sm text-slate-400">
                          <span className="capitalize">{practice.type || 'Mobile'}</span>
                          <span>•</span>
                      ...
                      <ArrowRight className="w-5 h-5 text-slate-600 group-hover:text-cyan-400 transition-colors ml-4 flex-shrink-0" />
                    </div>
                  </div>
                </Link>
              </motion.div>
