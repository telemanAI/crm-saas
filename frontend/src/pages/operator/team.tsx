...
export default function TeamPage() {
  ...
  return (
    <OperatorLayout title="Team">
      ...
      <AnimatePresence>
        {inviteOpen ? (
          <InviteDialog onClose={() => setInviteOpen(false)} onDone={() => { setInviteOpen(false); load(); setTab('invites'); }} />
        ) : null}
        {removeDialog ? (
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {!m.isActive ? <span className="text-[10px] bg-rose-500/20 text-rose-300 px-2 py-0.5 rounded uppercase font-bold">Ex-operatore</span> : null}
          </div>
...
function InviteCard({ inv, onResend, onRevoke }: InviteCardProps) {
...
function InviteDialog({ onClose, onDone }: InviteDialogProps) {
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={(e) => e.stopPropagation()} className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-md w-full">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            ...
            <select value={role} onChange={(e) => setRole(e.target.value as 'OPERATOR' | 'ADMIN')} data-testid="invite-role-select" className="w-full bg-slate-950/50 border border-slate-800 rounded-xl px-4 py-2.5 text-slate-200 text-sm focus:outline-none">
...
function RemoveDialog({ m, onClose, onDone }: RemoveDialogProps) {
